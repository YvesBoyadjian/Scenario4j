...
public class SoVolumetricShadowGroupP extends SoShadowGroupP {
    ...
    protected void endShadowLight(SoShaderGenerator gen,int index,int texunit,int shadowlightnumber) {
        ...
        gen.addMainStatement("    float densityMie = densityAtPoint(pos.xyz, 10);");
        gen.addMainStatement("    scatter = scatter * shadeFactor;");
        gen.addMainStatement("    accumFog += vec3(scatter,scatter,scatter) * g_SunColor * 2.4 * densityMie;");
        gen.addMainStatement("    scatter = ComputeRayleighScattering(cosinusRaySun);");
        gen.addMainStatement("    float densityRayleigh = densityAtPoint(pos.xyz, 1);");
        gen.addMainStatement("    accumFog += vec3("+RED_FACTOR+","+GREEN_FACTOR+","+BLUE_FACTOR+") * g_SunColor * shadeFactor * densityRayleigh * 0.5;");
        gen.addMainStatement("  }");
