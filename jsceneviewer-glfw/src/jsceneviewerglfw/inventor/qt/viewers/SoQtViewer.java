...
protected void processEvent (TypedEvent anyEvent, EventType type, final boolean[] isAccepted)
{
    ...
    if (type == EventType.KEY_EVENT_KEY_PRESSED) {
        ...
        if (keyCommandsEnabled && (ke.keyCode == GLFW_KEY_ESCAPE/* Qt::Key_Escape*/)) {
            setViewing( !isViewing());  // toggle the viewing mode...
            isAccepted[0] = true;//ke.accept();
        } else if (altSwitchingEnabled && !altSwitchBack && (ke.keyCode == SWT.ALT/*Qt::Key_Alt*/)) {
        ...
        int modifiers = ke.stateMask & SWT.MODIFIER_MASK;
        int buttons = ke.stateMask & SWT.BUTTON_MASK;
        if (altSwitchBack && (ke.keyCode == SWT.ALT/*Qt::Key_Alt*/)) {
        ...
        // the window.
        int modifiers = ((MouseEvent)anyEvent).stateMask & SWT.MODIFIER_MASK;//QApplication::keyboardModifiers();
        int buttons = ((MouseEvent)anyEvent).stateMask & SWT.BUTTON_MASK;
        if (altSwitchingEnabled && !altSwitchBack && (modifiers & SWT.ALT/*Qt::AltModifier*/)!=0) {
            altSwitchBack = true;   // later return back
            setViewing(!isViewing());
        } else if (altSwitchBack && (modifiers & SWT.ALT/*Qt::AltModifier*/)==0) {
            setViewing(!isViewing());
            altSwitchBack = false;  // clear the flag
        }
        ...
        !isAccepted[0] && type == EventType.KEY_EVENT_KEY_PRESSED)
    {
        ...
        switch ( ke.keyCode ) {
            ...
            case GLFW_KEY_RIGHT:
            case GLFW_KEY_DOWN:
                getCameraController().arrowKeyPressed (ke.keyCode);
