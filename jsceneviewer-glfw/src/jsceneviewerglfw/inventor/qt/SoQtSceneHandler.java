...
import jsceneviewerglfw.inventor.qt.SoQtStandardEventTranslator.LocationRefreshMouseEvent;
...
public abstract class SoQtSceneHandler {
    ...
    private Composite lastEventWidget;
...
//! override this to enable/disable devices for the correct widget
	protected Composite getDeviceWidget() { return null; }
...
setAutoRedraw(boolean flag)
{
    ...
    public void resendLastMousePosition() {
        if (lastEventPositionValid && (lastPositionEventHandler == this)) {
        	...
        	e.x = lastEventPosition.getX();
        	e.y = lastEventPosition.getY();
        	e.stateMask = storedModifierState +	storedButtonState;
            MouseEvent me = new LocationRefreshMouseEvent(e);
...
boolean removeSceneCallback (SoCallback callbackNode)
{
    ...
    public void processSceneEvent (TypedEvent anyEvent, EventType type) {
        ...
        {
            isKeyEvent = true;
        } else if (anyEvent instanceof MouseEvent && !(anyEvent instanceof LocationRefreshMouseEvent))
        {
            isMouseEvent = true;
        }
        ...
        if (isMouseEvent && (type != EventType.MOUSE_EVENT_MOUSE_MOVE)) {
          ...
          if (( !Objects.equals(new Point(me.x,me.y), lastEventPosition)) ||
              !lastEventPositionValid || (lastPositionEventHandler == null))
          {
            ...
            MouseEvent synthEvent = new MouseEvent(e);
        ...
        if (isKeyEvent || isMouseEvent) {
        	int modifier = 0;
        	if(isKeyEvent) {
        		modifier = ((KeyEvent)anyEvent).stateMask & SWT.MODIFIER_MASK;
        	}
        	else if(isMouseEvent) {
        		modifier = ((MouseEvent)anyEvent).stateMask & SWT.MODIFIER_MASK;        		
        	}
            synthesizeCurrentModifierState (modifier, anyEvent.widget);
        }
        ...
        if (isKeyEvent || isMouseEvent) {
        	if(isKeyEvent) {
        		storedModifierState = ((KeyEvent)anyEvent).stateMask & SWT.MODIFIER_MASK;
        	}
        	else if(isMouseEvent) {
        		storedModifierState = ((MouseEvent)anyEvent).stateMask & SWT.MODIFIER_MASK;        		
        	}
            ...
            if (isKeyEvent) {
                ...
                if (ke.keyCode == SWT.SHIFT) {
                    storedModifierState ^= SWT.SHIFT;
                }
                if (ke.keyCode == SWT.CONTROL) {
                    storedModifierState ^= SWT.CONTROL;
                }
                if (ke.keyCode == SWT.ALT) {
                    storedModifierState ^= SWT.ALT;
                }
            }
        }
...
void synthesizeCurrentModifierState (int modifiers, Composite widget)
{
    if (modifiers != storedModifierState) {
        // first generate key release events for every modifier that is different:
        synthesizeModifierState (modifiers, SWT.SHIFT,   SWT.SHIFT,   false, widget);
        synthesizeModifierState (modifiers, SWT.CONTROL, SWT.CONTROL, false, widget);
        synthesizeModifierState (modifiers, SWT.ALT,     SWT.ALT,     false, widget);
        // then generate key press events for every modifier that is different:
        synthesizeModifierState (modifiers, SWT.SHIFT,   SWT.SHIFT,   true, widget);
        synthesizeModifierState (modifiers, SWT.CONTROL, SWT.CONTROL, true, widget);
        synthesizeModifierState (modifiers, SWT.ALT,     SWT.ALT,     true, widget);
    }
}
...
void synthesizeModifierState (int newModifiers,
                                           int modifier,
                                           int keyCode, boolean press, Composite widget)
{
    // generate a button press or release event if the button state of the selected button
    // is different and the new state correlates with the "press" flag:
        ...
        (((newModifiers & modifier) != 0) == press))
    {
    	...
    	e.widget = widget;
    	e.keyCode = keyCode;
    	e.stateMask = storedModifierState;
        KeyEvent synthEvent = new KeyEvent(e);
...
void translateAndSendEvent(TypedEvent anyevent, EventType type)
{
...
SoEvent translateEvent(TypedEvent anyEvent, EventType type)
{
    // send event to the scene
...
SoEvent translateStdEvent(TypedEvent xe, EventType type)
{
